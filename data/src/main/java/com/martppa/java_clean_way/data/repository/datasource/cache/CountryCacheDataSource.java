...
package com.martppa.java_clean_way.data.repository.datasource.cache;
...
public class CountryCacheDataSource implements CountryDataSource {
    private CountryCache cache;

    @Inject
    public CountryCacheDataSource(CountryCache cache) {
        this.cache = cache;
    }
    ...
    public Observable<Collection<CountryEntity>> getCountries() {
        return Observable.create(emitter -> {
            CountryEntity[] countryEntities = cache.get();
            emitter.onNext(Arrays.asList(countryEntities));
            emitter.onComplete();
        });
    }

    public boolean isExpired() {
        return cache.hasExpired();
    }
}
